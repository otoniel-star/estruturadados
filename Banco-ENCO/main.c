...
int compare(const void *a, const void *b) {
    ...
    Cliente *binary_search_by_numero_conta(Cliente *clientes, int total_clientes, int numero_conta) 
    {
        int left = 0, right = total_clientes - 1;
    
        while (left <= right) {
            int mid = left + (right - left) / 2;
            int cmp = clientes[mid].contas_associadas.numero_conta - numero_conta;
    
            if (cmp == 0) {
                return &clientes[mid];  // Cliente encontrado
            } else if (cmp < 0) {
    ...
    Cliente *binary_search_by_cpf(Cliente *clientes, int total_clientes, const char *cpf_busca) 
    {
        int left = 0, right = total_clientes - 1;
        ...
        {
            int mid = left + (right - left) / 2;
            int cmp = strcmp(clientes[mid].cpf, cpf_busca);
...
void criar_conta(Cliente *clientes, int *total_clientes) {
    ...
    void depositar(Cliente *clientes, int total_clientes) 
    {
        ...
        Cliente *cliente = binary_search_by_numero_conta(clientes, total_clientes, numero_conta);
    
        if (cliente != NULL) {
            printf("Digite o valor a ser depositado: ");
            scanf("%f", &valor);
            cliente->contas_associadas.saldo += valor;
            printf("Depósito realizado com sucesso. Saldo atual: %.2f\n", cliente->contas_associadas.saldo);
        } else {
    ...
    void sacar(Cliente *clientes, int total_clientes) 
    {
        ...
        if (cliente != NULL) {
            ...
            scanf("%f", &valor);
            if (cliente->contas_associadas.saldo >= valor) {
                cliente->contas_associadas.saldo -= valor;
    ...
    void consultar_saldo(Cliente *clientes, int total_clientes) 
    {
        ...
        if (cliente != NULL) {
            printf("Saldo da conta %d: %.2f\n", cliente->contas_associadas.numero_conta, cliente->contas_associadas.saldo);
        } else {
            printf("Conta não encontrada.\n");
        }
    }
...
void buscar_cliente_por_cpf(Cliente *clientes, int total_clientes) {
    ...
    // Chamar a função de busca binária
    Cliente *result = binary_search_by_cpf(clientes, total_clientes, cpf_busca);
...
int main() {
    Cliente clientes[NUM_CONTAS];  // Podemos ter no máximo NUM_CONTAS clientes
